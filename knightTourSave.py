...
def hamiltonian_path(n,graph, pos, path, nelPath, move_cnt,nomeFile,asTab,id=0):
    path.append(pos)
    nelPath[pos]=1
    save_board(nelPath,n,nomeFile,id,asTab)
    ...
    if not update_cnt(n,x,y,move_cnt,nelPath,dec=True) or deltalen == 1:
        ...
        for neighbor in neighbor_list:
            extended_path = hamiltonian_path(n,graph, neighbor, path,nelPath,move_cnt,nomeFile,asTab,id)
            if extended_path: 
...
def percorsoCavallo(n,asTab=False):
    graf = creaGrafo(n)
    used = [0]*(n*n)
    move_cnt = make_cnt(n)
    ...
    with open(nomeRicerca, 'w') as f:
        f.write('\n')
    sol = hamiltonian_path(n,graf,0,[],used,move_cnt,nomeRicerca,asTab=asTab,id=0)
    return sol
...
if __name__ == '__main__':
    percorsoCavallo(376,asTab=False)
